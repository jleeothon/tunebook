---
name: indent-tune
description: Re-indent the body of a single tune in an .abc tunebook so measures align to a column grid. Use when the user types /indent-tune <X> or asks to indent a specific tune by its X-number.
---
...
User provides a tune index (e.g. `73`, `X:73`). Default file: `irish tunebook.abc` in the current working directory. If multiple `.abc` files are open or modified, ask which. If no tune is given, ask.

## Procedure

1. **Find the tune.** Locate `X:\s*<N>\b` in the file. The body runs from the first line after the header block (X / T / C / R / M / L / K / Q etc.) until the next blank line or next `X:`.
